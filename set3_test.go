...
func TestChallenge21(t *testing.T) {
	...
	mt := newMT19937FromArray(t, []uint32{0x123, 0x234, 0x345, 0x456})
	for i, num := range mt19937TestVector {
		gen := mt.generate()
		if gen != num {
			t.Fatalf("%03d: expected %d, got %d", i, num, gen)
		}
	}
}
